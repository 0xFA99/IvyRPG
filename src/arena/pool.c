...
{
    IVY_ASSERT(pool != NULL, "pool is NULL");
    IVY_ASSERT(chunk_size > 0, "chunk_size must be > 0");
    IVY_ASSERT(chunk_count > 0, "chunk_count must be > 0");
...
{
    IVY_ASSERT(pool != NULL, "pool is NULL");
    IVY_ASSERT(buf != NULL, "buf is NULL");
    IVY_ASSERT(buf_size > 0, "buf_size must be > 0");
    IVY_ASSERT(chunk_size > 0, "chunk_size must be > 0");
...
{
    IVY_ASSERT(pool != NULL, "pool is NULL");
    IVY_ASSERT(ptr != NULL, "ptr is NULL");
    IVY_ASSERT(IsArenaPoolOwned(pool, ptr), "ptr not owned by pool");
